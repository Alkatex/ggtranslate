...
export async function postTranslation(opts: {
  ...
  try {
    ...
    const embed = new EmbedBuilder()
      .setColor(0x06b6d4)
      .setDescription(
        `${opts.sourceLangFlag} **${opts.original}**\n${opts.targetLangFlag} ${opts.translated}`
      )
      .setFooter({
        text: `${opts.game ? `${opts.gameEmoji} ${opts.game} · ` : ''}GGTranslate${opts.username ? ` · ${opts.username}` : ''} · ggtranslate.com`,
      })
      .setTimestamp()
...
// ─── Vérifier si le bot est dans un serveur ───────────────────────────────────
