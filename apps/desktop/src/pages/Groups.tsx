    ...
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '24px' }}>
        <div>
          ...
          <p style={{ color: '#475569', fontSize: '11px', letterSpacing: '0.15em', marginTop: '2px' }}>GROUPES VOCAUX</p>
        </div>
      ...
      {success && (
        <div style={{ background: 'rgba(34,197,94,0.1)', border: '1px solid #22c55e', borderRadius: '8px', padding: '10px 14px', marginBottom: '16px', color: '#22c55e', fontSize: '13px' }}>
          {success}
        </div>
      )}
      ...
      {activeGroup && (
        <div style={{ background: 'rgba(6,182,212,0.05)', border: '1px solid #06b6d4', borderRadius: '12px', padding: '20px', marginBottom: '24px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
            <div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                ...
                <button onClick={() => copyCode(activeGroup.code)} style={{
                  background: copiedCode === activeGroup.code ? 'rgba(34,197,94,0.2)' : 'rgba(6,182,212,0.1)',
                  border: 'none', color: copiedCode === activeGroup.code ? '#22c55e' : '#06b6d4',
          ...
          <div style={{ fontFamily: 'Orbitron, sans-serif', fontSize: '10px', color: '#475569', letterSpacing: '0.1em', marginBottom: '10px' }}>
            👥 MEMBRES ({activeMembers.length}/{activeGroup.max_members})
          </div>
            ...
            {activeMembers.length === 0 ? (
              ...
              activeMembers.map(member => (
                <div key={member.id} style={{
                  background: member.is_speaking ? 'rgba(34,197,94,0.15)' : '#111827',
                  border: `1px solid ${member.is_speaking ? '#22c55e' : '#1e2d45'}`,
                  borderRadius: '8px', padding: '6px 12px',
                  display: 'flex', alignItems: 'center', gap: '6px',
                }}>
                  <span style={{ fontSize: '14px' }}>{LANG_FLAGS[member.language] || '🌍'}</span>
                  <span style={{ color: member.user_id === user?.id ? '#06b6d4' : '#94a3b8', fontSize: '11px' }}>
                    {member.user_id === user?.id ? 'Toi' : 'Joueur'}
                  </span>
          ...
          <div style={{
            background: '#0a0f1a', borderRadius: '10px',
            ...
            {messages.length === 0 ? (
              <div style={{ color: '#475569', fontSize: '12px', textAlign: 'center', marginTop: '80px' }}>
              ...
              messages.map(msg => (
                <div key={msg.id} style={{
                  ...
                  <div style={{
                    background: msg.type === 'translation'
                      ? 'rgba(168,85,247,0.15)'
                      : msg.user_id === user?.id
                        ? 'rgba(6,182,212,0.15)'
                        : '#111827',
                    border: `1px solid ${msg.type === 'translation' ? '#a855f7' : msg.user_id === user?.id ? '#06b6d4' : '#1e2d45'}`,
                    borderRadius: '10px', padding: '8px 12px',
                    maxWidth: '80%',
                  }}>
                    {msg.type === 'translation' && (
                      <div style={{ color: '#a855f7', fontSize: '9px', fontFamily: 'Orbitron, sans-serif', marginBottom: '4px', letterSpacing: '0.1em' }}>
                        🎤 TRADUCTION VOCALE {LANG_FLAGS[msg.language] || '🌍'}
                      </div>
                    )}
                    <div style={{
                      color: msg.type === 'translation' ? '#e9d5ff'
                        : msg.user_id === user?.id ? '#06b6d4' : '#fff',
                      fontSize: '13px',
                    }}>{msg.message}</div>
                  </div>
                </div>
          ...
          <div style={{ display: 'flex', gap: '8px' }}>
            ...
            <button onClick={sendMessage} disabled={!newMessage.trim()} style={{
              ...
              fontSize: '16px', opacity: newMessage.trim() ? 1 : 0.5,
            }}>➤</button>
          </div>
        </div>
      )}
      ...
      <div style={{ marginBottom: '20px' }}>
        <p style={{ color: '#94a3b8', fontSize: '11px', letterSpacing: '0.1em', marginBottom: '8px', fontFamily: 'Orbitron, sans-serif' }}>
          MA LANGUE DANS LES GROUPES
        </p>
        <select value={selectedLang} onChange={e => setSelectedLang(e.target.value)} style={{
          width: '100%', background: '#0d1424', border: '1px solid #1e2d45', color: '#fff',
          padding: '12px 16px', borderRadius: '10px', fontSize: '14px', cursor: 'pointer',
        }}>
      ...
      <div style={{ display: 'flex', gap: '12px', marginBottom: '20px' }}>
        <button onClick={() => { setShowCreate(!showCreate); setShowJoin(false) }} style={{
          flex: 1,
          background: showCreate ? 'linear-gradient(to right, #3b82f6, #06b6d4)' : 'transparent',
        ...
        <button onClick={() => { setShowJoin(!showJoin); setShowCreate(false) }} style={{
          flex: 1,
          background: showJoin ? 'rgba(6,182,212,0.15)' : 'transparent',
      ...
      {showCreate && (
        <div style={{ background: '#0d1424', border: '1px solid #1e2d45', borderRadius: '12px', padding: '20px', marginBottom: '20px' }}>
          <div style={{ fontFamily: 'Orbitron, sans-serif', fontSize: '11px', color: '#06b6d4', marginBottom: '16px' }}>✨ CRÉER UN GROUPE</div>
          <input
            value={groupName}
            onChange={e => setGroupName(e.target.value)}
            placeholder="Nom du groupe (ex: Valorant FR/EN)"
            style={{
              width: '100%', background: '#111827', border: '1px solid #1e2d45', color: '#fff',
          ...
          <button onClick={createGroup} disabled={loading || !groupName.trim()} style={{
            ...
            fontFamily: 'Orbitron, sans-serif', opacity: loading ? 0.7 : 1,
          }}>{loading ? '⟳ Création...' : 'Créer le groupe →'}</button>
        </div>
      )}
      ...
      {showJoin && (
        <div style={{ background: '#0d1424', border: '1px solid #1e2d45', borderRadius: '12px', padding: '20px', marginBottom: '20px' }}>
          <div style={{ fontFamily: 'Orbitron, sans-serif', fontSize: '11px', color: '#06b6d4', marginBottom: '16px' }}># REJOINDRE PAR CODE</div>
          <div style={{ display: 'flex', gap: '8px' }}>
            <input
              value={joinCode}
              ...
              style={{
                flex: 1, background: '#111827', border: '1px solid #1e2d45', color: '#fff',
                padding: '10px 14px', borderRadius: '8px', fontSize: '13px',
              }}
            />
            <button onClick={() => joinGroup()} disabled={loading} style={{
              background: 'linear-gradient(to right, #3b82f6, #06b6d4)',
      ...
      {myGroups.length > 0 && (
        <div style={{ marginBottom: '20px' }}>
          <p style={{ color: '#94a3b8', fontSize: '11px', letterSpacing: '0.1em', marginBottom: '12px', fontFamily: 'Orbitron, sans-serif' }}>
            ⭐ MES GROUPES
          </p>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
        {publicGroups.length === 0 ? (
          <div style={{ border: '1px dashed #1e2d45', borderRadius: '12px', padding: '40px', textAlign: 'center', color: '#475569', fontSize: '14px' }}>
            Aucun groupe public — sois le premier à en créer un !
          </div>
        ) : (
          publicGroups.map(group => (
            <div key={group.id} style={{
              ...
              <button onClick={() => joinGroup(group.code)} style={{
                background: 'transparent', border: '1px solid #06b6d4',
                color: '#06b6d4', padding: '8px 16px', borderRadius: '8px',
