...
interface ProfileCache {
  username: string; bio: string; mainLanguage: string
  avatarId: number; totalPhrases: number; totalSessions: number
}
...
export function ProfilePage() {
  ...
  const hasCache = _profileCache !== null && _profileCacheUserId === user?.id
  const [username, setUsername] = useState(hasCache ? _profileCache!.username : '')
  const [bio, setBio] = useState(hasCache ? _profileCache!.bio : '')
  const [mainLanguage, setMainLanguage] = useState(hasCache ? _profileCache!.mainLanguage : 'fr')
  const [avatarId, setAvatarId] = useState(hasCache ? _profileCache!.avatarId : 0)
  const [showAvatarPicker, setShowAvatarPicker] = useState(false)
  const [isSaving, setIsSaving] = useState(false)
  const [saveMsg, setSaveMsg] = useState('')
  const [totalPhrases, setTotalPhrases] = useState(hasCache ? _profileCache!.totalPhrases : 0)
  const [totalSessions, setTotalSessions] = useState(hasCache ? _profileCache!.totalSessions : 0)
    ...
    try {
      ...
      if (data) {
        const cached: ProfileCache = {
          ...
          bio: data.bio || '',
          mainLanguage: data.main_language || 'fr',
          avatarId: data.avatar_id ?? 0,
          totalPhrases: data.total_phrases || 0,
          totalSessions: data.total_sessions || 0,
        }
        _profileCache = cached
        _profileCacheUserId = user.id
        setUsername(cached.username)
        setBio(cached.bio)
        setMainLanguage(cached.mainLanguage)
        setAvatarId(cached.avatarId)
        setTotalPhrases(cached.totalPhrases)
  ...
  async function saveProfile() {
    ...
    try {
      ...
      if (error) {
        ...
        // Invalide le cache pour forcer un rechargement au prochain montage
        _profileCache = null
    ...
    <div style={{ position: 'relative', zIndex: 1, minHeight: '100vh', padding: '0 24px 24px', maxWidth: '800px', margin: '0 auto' }}>
            ...
            <div style={{ background: tokens.colors.bg2, border: `1px solid ${tokens.colors.border}`, borderRadius: tokens.radius['3xl'], padding: '20px' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: '12px' }}>
                {AVATARS.map((avatar, i) => (
                  <motion.div key={avatar.id} initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }}
                    transition={{ duration: 0.2, delay: i * 0.03 }} whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.95 }}
      ...
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3, delay: 0.22 }}
        ...
        <div style={{ marginBottom: '16px' }}>
          ...
          <input value={username} onChange={e => setUsername(e.target.value)} placeholder="MonPseudo123" maxLength={30}
        ...
        <div style={{ marginBottom: '16px' }}>
          ...
          <textarea value={bio} onChange={e => setBio(e.target.value)} placeholder="Joueur passionné de FPS..." maxLength={150} rows={3}
        ...
        <motion.button whileHover={isSaving ? {} : { scale: 1.01 }} whileTap={isSaving ? {} : { scale: 0.98 }}
